...
def cfar_global(img):
    ...
    # 数据中小于2倍均值的部分指的是灰度值小于图像灰度值的两倍均值的像素点。这里的目的是标记海面，
    # 因为海面通常是比较暗的区域，灰度值相对较低，而且相邻的像素点灰度值变化不会很大。因此，选取小于2倍均值的部分可以过滤掉一些灰度值变化比较大的像素点，更准确地标记出海面。
...
def cfar_shixin(img):
    ...
    for i in range(0, N - win_size, step_size):
        for j in range(0, M - win_size, step_size):
            ...
            # end = time.time()  # 记录结束时间
            # print(' all time: {}'.format(end - start))
            # mu = np.mean(window) # 计算窗口内杂波平均值
            # sigma = np.std(window) # 计算杂波标准差
            #
            # # 假设窗口内的像素值服从高斯分布，进行参数估计
            # T = mu + threshold * sigma # 计算检测阈值
            #
            # # 将窗口内的像素值与阈值进行比较,如果像素值大于阈值，表示该像素为目标像素
            # window[window < T] = 0
            # window[window >= T] = 255
            #
            # img[i:i+win_size, j:j+win_size] = window # 将检测结果保存到原始图像中

    end = time.time()  # 记录结束时间
    print(' all time: {}'.format(end - start))
    detections[detections != 0] = 1  # 最终二值化结果
    img_r = detections * 255
...
def detection_get_clutter(sar_data, clutter_win, protect_win, x, y):
    ...
    # data = np.array(data_out[data_out > 0])  # 选取数据中大于0的部分
    # sea = data[data < 2 * np.mean(data)]  # 将数据中小于2倍均值的部分标记为海面
    # sea_list = sea.tolist()  # 将数据转化为列表
    # t = min(len(sea_list), 10000)  # 选取最小值为10000和列表长度之间的较小值
    # data_out = np.random.choice(sea_list, size=t, replace=False)  # 从列表中随机选取t个样本作为一维数组
...
def detection_count_th(clutter_data, Pfa, distribution):
    ...
    if distribution == 'rayleigh':
        ...
        mean_val = np.mean(np.log(clutter_data + 0.000001))  # 估计分布参数，均值
        std_val = np.std(np.log(clutter_data + 0.000001))  # 方差
...
# 双参数CFAR检测
def detection_cfar(sar_data, clutter_win, protect_win, distribution, minShip, Pfa):
    """
    ...
    start = time.time()
    # print('CFAR检测开始')
    sar_data = sar_data.astype(np.float64)
